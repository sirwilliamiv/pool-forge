...
/**
 * The proposal share link, small enough to sit inside a card, a popover, or
 * the summary rail. Same behaviour everywhere: creating the link files the
 * sent proposal first (that is `shareProject`'s contract), revoking stops
 * acceptance.
 */
export function ShareLinkControl({
  ...
  React.useEffect(() => setToken(initialToken), [initialToken])
      ...
      {token ? (
        <>
          <div className="flex gap-2">
            ...
            />
            <Button type="button" variant="outline" size="sm" className="h-9" onClick={copy}>
              Copy
            </Button>
          </div>
        ...
        <div className={compact ? 'space-y-2' : 'flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between'}>
          <p className="text-bodyS text-theme-muted">
            Create a private link the customer can open to view and accept this proposal. No sign-in
            required.
          </p>
