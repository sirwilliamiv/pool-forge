# Site import: address to scaled backdrop, parcel, and building
...
2. If Regrid is configured: draws the parcel boundary as the existing
   `symbol.property-line` stencil shape and autofills `Project.parcelId` and
   `Project.jurisdiction`.
3. If the Solar API knows the building: draws the house footprint as shapes.
...
- Google has no parcel API; parcels come from Regrid (optional, keyed) or not
  at all. Assessor lines are tax-map approximations; the UI says so.
...
- `regrid.ts`: server client for parcel-by-point. Reads `REGRID_API_KEY`,
  optional, same degrade rule.
...
- `site.address.set` (server): input placeId or {address, lat, lng}; resolves
  details, writes `Project.siteAddress/sitePlaceId/latitude/longitude`, and,
  when Regrid answers, `parcelId`/`jurisdiction`.
...
- `site.import.parcel` (server): fetches parcel polygon, projects to editor
  inches via `mercator.ts`, appends a `symbol.property-line` shape to
  `rootJson.shapes` in one `db.$transaction`, exactly the
  `import.intent.apply` append pattern.
- `site.import.building` (server): Solar footprint → house shapes, same
  append pattern.
...
- `REGRID_API_KEY`: Secret Manager `pool-forge-regrid-api-key` (not yet
  created; Billy signs up when parcels should go live).
...
grades; Nearmap-quality licensed imagery; parcel setback auto-population
beyond what Regrid returns.
